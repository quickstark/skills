...
- Run the `/qs-design-modules` skill for the architecture vocabulary (**module**, **interface**, **depth**, **seam**, **adapter**, **leverage**, **locality**) and its principles (the deletion test, "the interface is the test surface", "one adapter = hypothetical seam, two = real"). Use these terms exactly in every suggestion — don't drift into "component," "service," "API," or "boundary."
...
Write any standalone architecture-review source to the OS temp directory so nothing lands in the repository, but treat its absolute path as private implementation evidence, never as the user's readout or website. Capture the actual candidates, decisions, diagrams, and checks in the canonical QuickStark architecture report and publish that report with `scripts/qs-skill-readout.mjs render --require-hosted`. Present only the authenticated `https://reports.quickstark.com/` URL returned after verified producer delivery. Never open a temporary HTML path, localhost address, private IP, or editor attachment as the primary report. If the selected Mac, Linux, Windows, or ChatGPT producer has no valid private token, fail clearly and preserve the local source without presenting it as a hosted report. Include a separate standalone visual only when its actual published URL is also on `reports.quickstark.com`; otherwise the hosted, purpose-built architecture readout is the visual report.
...
**Use CONTEXT.md vocabulary for the domain, and the `/qs-design-modules` vocabulary for the architecture.** If `CONTEXT.md` defines "Order," talk about "the Order intake module" — not "the FooBarHandler," and not "the Order service."
...
Once the user picks a candidate, run the `/qs-plan-interview` skill to walk the decision tree with them — constraints, dependencies, the shape of the deepened module, what sits behind the seam, what tests survive.

Side effects happen inline as decisions crystallize — run the `/qs-design-domain` skill to keep the domain model current as you go:

- **Naming a deepened module after a concept not in `CONTEXT.md`?** Add the term to `CONTEXT.md`. Create the file lazily if it doesn't exist.
- **Sharpening a fuzzy term during the conversation?** Update `CONTEXT.md` right there.
- **User rejects the candidate with a load-bearing reason?** Offer an ADR, framed as: _"Want me to record this as an ADR so future architecture reviews don't re-suggest it?"_ Only offer when the reason would actually be needed by a future explorer to avoid re-suggesting the same thing — skip ephemeral reasons ("not worth it right now") and self-evident ones.
- **Want to explore alternative interfaces for the deepened module?** Run the `/qs-design-modules` skill and use its design-it-twice parallel sub-agent pattern.
...
>
> Heuristic: Module design benefits from carefully reasoning about interfaces and seams. Never change the active model or thinking level.
