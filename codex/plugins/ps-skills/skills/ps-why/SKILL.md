...
Label prompts `Preferred next prompt:` and `Alternative next prompt:`. Put each in its own fenced `text` block, beginning with its exact Codex literal ($qs-skills:qs-plan-clarify, $ps-skills:ps-how, $ps-skills:ps-blast-radius, $qs-skills:qs-flow-handoff); Claude uses `/qs-plan-clarify`, `/ps-how`, `/ps-blast-radius`, `/qs-flow-handoff`; Pi uses `/skill:qs-plan-clarify`, `/skill:ps-how`, `/skill:ps-blast-radius`, `/skill:qs-flow-handoff`. Carry forward only the outcome and highest-value evidence. Keep model guidance outside the fence and never change the active model or reasoning setting.
