...
Label prompts `Preferred next prompt:` and `Alternative next prompt:`. Put each in its own fenced `text` block, beginning with its exact Codex literal ($qs-specialists:qs-skill-write, $qs-skills:qs-plan-clarify, $qs-skills:qs-flow-handoff); Claude uses `/qs-skill-write`, `/qs-plan-clarify`, `/qs-flow-handoff`; Pi uses `/skill:qs-skill-write`, `/skill:qs-plan-clarify`, `/skill:qs-flow-handoff`. Carry forward only the outcome and highest-value evidence. Keep model guidance outside the fence and never change the active model or reasoning setting.
