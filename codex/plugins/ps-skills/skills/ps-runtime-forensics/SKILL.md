...
If durable instrumentation or a product repair is needed, return `continuation-required` before changing tracked product source. Never include sensitive artifact contents or private paths in the hosted report.

## Completion report and next steps

This invocation has one root skill: `/ps-runtime-forensics`. Internal capabilities and bounded helpers remain part of this run; never automatically invoke another public skill or create another skill report.
...
The default ranked continuations are `/qs-code-debug`, `/ps-trace-forensics`, `/qs-flow-handoff`. A failed result instead ranks `/qs-code-debug`, `/qs-flow-handoff`, `/ps-trace-forensics`. Tailor each prompt to the actual result instead of starting it.
...
Present prompts in normalized rank order. Label the first `Preferred next prompt:` and the remaining two `Alternative next prompt:`. Put each complete prompt in its own fenced `text` block beginning with its exact Codex skill literal ($qs-skills:qs-code-debug, $ps-skills:ps-trace-forensics, $qs-skills:qs-flow-handoff); Claude uses `/qs-code-debug`, `/ps-trace-forensics`, `/qs-flow-handoff`. The fence info string must be exactly `text` so the chat renders it as Plain text; never use `markdown`, `bash`, `json`, or another language. Keep every prompt concise and carry forward only the outcome plus the single highest-value evidence item. Put heuristic model/thinking guidance outside each fence in a muted blockquote. Never change the active model or reasoning setting.
