...
Label prompts `Preferred next prompt:` and `Alternative next prompt:`. Put each in its own fenced `text` block, beginning with its exact Codex literal ($qs-specialists:qs-plan-research, $qs-specialists:qs-design-prototype, $qs-specialists:qs-skill-write); Claude uses `/qs-plan-research`, `/qs-design-prototype`, `/qs-skill-write`; Pi uses `/skill:qs-plan-research`, `/skill:qs-design-prototype`, `/skill:qs-skill-write`. Carry forward only the outcome and highest-value evidence. Keep model guidance outside the fence and never change the active model or reasoning setting.
