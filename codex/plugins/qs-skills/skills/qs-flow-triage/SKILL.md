...
Label prompts `Preferred next prompt:` and `Alternative next prompt:`. Put each in its own fenced `text` block, beginning with its exact Codex literal ($qs-skills:qs-plan-roadmap, $qs-skills:qs-code-debug, $qs-skills:qs-code-build); Claude uses `/qs-plan-roadmap`, `/qs-code-debug`, `/qs-code-build`; Pi uses `/skill:qs-plan-roadmap`, `/skill:qs-code-debug`, `/skill:qs-code-build`. Carry forward only the outcome and highest-value evidence. Keep model guidance outside the fence and never change the active model or reasoning setting.
