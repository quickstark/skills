...
Default routes: `/qs-git-merge`, `/qs-code-build`, `/qs-code-debug`. Failure routes: `/qs-code-build`, `/qs-code-debug`, `/qs-flow-handoff`. Tailor every prompt to the completed work.
...
Label prompts `Preferred next prompt:` and `Alternative next prompt:`. Put each in its own fenced `text` block, beginning with its exact Codex literal ($qs-skills:qs-git-merge, $qs-skills:qs-code-build, $qs-skills:qs-code-debug, $qs-skills:qs-flow-handoff); Claude uses `/qs-git-merge`, `/qs-code-build`, `/qs-code-debug`, `/qs-flow-handoff`. Carry forward only the outcome and highest-value evidence. Keep model guidance outside the fence and never change the active model or reasoning setting.
