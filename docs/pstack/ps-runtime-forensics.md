...
`/ps-runtime-forensics` produces one normalized root result directly in chat. It accepts independent `effort=quick|standard|deep` and `report=brief|full` modes, defaulting to `standard` and `brief`.
...
Eligible normal routes are `/qs-code-debug`, `/ps-trace-forensics`, `/qs-flow-handoff`. Failure routes are `/qs-code-debug`, `/qs-flow-handoff`, `/ps-trace-forensics`. Select at most one route that owns verified unfinished work.
