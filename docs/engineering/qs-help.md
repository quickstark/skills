...
`/qs-help` produces one normalized root result and one authenticated hosted readout. It accepts independent `effort=quick|standard|deep` and `report=brief|full` modes, defaulting to `standard` and `brief`.
...
The catalog-approved continuation, only when the result requires it, is `/qs-setup`.
