...
`qs-test-author` owns test-focused mutation for already-established behavior. `qs-test-verify` owns read-only execution and reporting of a selected verification matrix. Keeping them separate preserves clear mutation authority, completion criteria, failure handling, and result signals.
