...
| `/qs-test-author` | Established behavior covered, actual test artifacts, scope decisions, and focused and wider validation. | Test artifact and validation checklist using only checks that actually ran. | Show a tested commit or related PR only when directly verified. |
| `/qs-test-verify` | The agreed verification matrix and actual pass, fail, skipped, or blocked result for each target. | Matrix of actual suites and environments without inferred results. | Show the tested commit or related PR only when directly verified. |
